...
export default function LibraryScreen() {
  ...
  const handlePlayAll = useCallback(() => {
    if (sortedTracks.length > 0) controls.playTrack(sortedTracks[0], sortedTracks);
  }, [controls, sortedTracks]);
  ...
  const getItemLayout = useCallback(
    (_: any, index: number) => ({ length: Layout.trackItemHeight, offset: Layout.trackItemHeight * index, index }),
    [],
  );
    ...
    <View style={styles.header}>
      ...
      <View style={styles.sortContainer}>
        {sortOpts.map(({ key, label }) => (
          <TouchableOpacity
            key={key}
            style={[styles.sortButton, { backgroundColor: sortBy === key ? c.primary : c.backgroundHighlight }]}
          ...
          ListEmptyComponent={
            isScanning ? (
              <View style={styles.emptyState}>
                ...
                <Text style={[styles.emptyText, { color: c.textSecondary }]}>Cargando...</Text>
              </View>
            ) : (
              <View style={styles.emptyState}>
                <Ionicons name="musical-notes" size={64} color={c.textMuted} />
                <Text style={[styles.emptyText, { color: c.textSecondary }]}>Sin canciones</Text>
              </View>
            )
          }
